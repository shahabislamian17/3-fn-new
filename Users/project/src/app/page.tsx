  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const usersQuery = query(collection(firestore, "users"), where("role", "==", "Investor"));
        ...
        const liveProjects = projectsData.filter(p => p.status === 'live');
        
        const totalRaised = projectsData.reduce((sum, p) => sum + (p.raisedAmount || 0), 0);
        const fundedProjectsCount = projectsData.filter(p => p.raisedAmount >= p.targetAmount).length;
        const uniqueCountries = new Set(projectsData.map(p => p.location)).size;
        const totalInvestors = usersSnapshot.size;

        const landingPageData: LandingData = {
          stats: {
            countries: uniqueCountries,
            investedUSD: formatLargeNumber(totalRaised),
              ...
              <Card>
                <CardContent className="p-6">
                   {loading || !landingData ? (
                     ...
                     <div className="flex flex-wrap gap-x-8 gap-y-4 justify-around text-center">
                        ...
                        <div className="flex flex-col">
                          <span className="text-3xl font-bold text-primary">📈 {landingData.stats.projectsFunded}</span>
                          <span className="text-sm text-muted-foreground">Projects Funded</span>
                        </div>
                         <div className="flex flex-col">
                          <span className="text-3xl font-bold text-primary">👥 {landingData.stats.totalInvestors}</span>
