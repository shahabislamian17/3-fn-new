...
function VerificationTab() {
  ...
  const isProjectOwner = appUser?.role === 'ProjectOwner';
  ...
  const handleInvestorProfileSubmit = async (data: any) => {
    if (!user) return;
    try {
      await apiRequest('/user/profile', { method: 'PATCH', body: data });
  ...
  return (
    <Card>
      <CardHeader>
          ...
          <CardDescription>Complete your identity and profile verification to unlock all platform features.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
          <PersonalDataForm userType={isProjectOwner ? 'owner' : 'investor'} onSubmit={handleKycSubmit} />
          {!isProjectOwner && <InvestorProfileForm user={appUser} onSubmit={handleInvestorProfileSubmit} />}
          {isProjectOwner && <BusinessProfileForm onSubmit={handleKycSubmit} />}
      </CardContent>
    </Card>
  )
}
...
export default function AccountSettingsPage() {
  ...
  if (!appUser) return <Loader2 className="h-8 w-8 animate-spin mx-auto my-12" />;

  const displayName = appUser.name || appUser.email || 'User';
  
  const coverImage = getPlaceholderImage(appUser.coverImageHint || 'fallback');
      ...
      <div className="relative h-48 w-full rounded-lg bg-card group">
        ...
        <div className="absolute bottom-4 left-4 flex items-end gap-4">
          <div className="relative">
           ...
           <div className="pb-2">
              ...
              <p className="text-sm text-gray-200 [text-shadow:0_1px_2px_rgb(0_0_0_/_0.5)]">{appUser.email}</p>
          </div>
        </div>
       ...
       <Tabs defaultValue={defaultTab}>
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                             ...
                             <FormDescription>
                                This determines which payout methods are available to you.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
        ...
        <TabsContent value="payouts">
            {isProjectOwner ? (
                appUser && <StripeOnboard user={appUser} />
            ) : appUser.payoutBlocked ? (
                 <Card>
                    <CardHeader>
                        <CardTitle>Payouts Blocked</CardTitle>
                        <CardDescription>Additional verification is required to enable payouts.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Alert variant="destructive">
                            ...
                            <AlertDescription>
                                Your account is not eligible for automated payouts via Stripe. Please complete our manual verification process to enable payouts.
                            </AlertDescription>
                        </Alert>
                        <Button asChild className="mt-4">
                            <Link href="/dashboard/fallback-kyc">Start Manual Verification</Link>
                        </Button>
                    </CardContent>
                </Card>
            ) : (
                ...
                <CardContent>
                   <Form {...settingsForm}>
                        <form onSubmit={settingsForm.handleSubmit(onSettingsSubmit)} className="space-y-8">
                            ...
                            <FormField control={form.control} name="emailNotifications" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4"><div className="space-y-0.5"><FormLabel className="text-base">Email Notifications</FormLabel><p className="text-sm text-muted-foreground">Receive emails about project updates and platform news.</p></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
