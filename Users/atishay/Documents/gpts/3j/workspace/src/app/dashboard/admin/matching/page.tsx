...
export default function MatchDashboard() {
  ...
  useEffect(() => {
    async function fetchProjects() {
        ...
        setLoading(true);
        try {
            const getProjectsCallable = httpsCallable(functions, 'getProjects');
            const result: any = await getProjectsCallable();
  ...
  async function fetchMatches(projectId: string) {
    ...
    setMatches([]);
    try {
      // In a real app, this would be a proxied function call.
      // For this demo, we are showing an empty state.
      setMatches([]);
    } catch (err: any) {
  ...
  if (loading) {
      return (
          <div className="flex justify-center items-center h-48 text-gray-600 dark:text-gray-300">
             <Loader2 className="animate-spin w-8 h-8 mr-2" /> Loading projects...
          </div>
      )
  }
