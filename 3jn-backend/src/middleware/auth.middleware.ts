...
export const protect = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (sessionCookie) {
    try {
        ...
        req.user = await getUserFromClaims(decodedClaims);
        return next();
    } catch (error) {
  ...
  return res.status(401).json({ error: 'Unauthorized: No session cookie or token' });
};

async function getUserFromClaims(decodedClaims: any): Promise<User> {
