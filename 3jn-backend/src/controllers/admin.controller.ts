    ...
    try {
        ...
        for (const project of activeProjects) {
            ...
            for (const investor of investors) {
                 const investmentsSnapshot = await adminDb.collection('investments').where('userId', '==', investor.id).get();
                 const portfolio = investmentsSnapshot.docs.map(doc => {
                    const investment = doc.data();
                    const investedProject = activeProjects.find(p => p.id === investment.projectId);
                    return {
